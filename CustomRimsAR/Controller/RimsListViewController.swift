            ...
            if let e = error {
                ...
                if let snapshotDocuments = querySnapshot?.documents {
                    for doc in snapshotDocuments {
                        let data = doc.data()
                        if let rimId = data["id"] as? Int,
                           let rimName = data["name"] as? String,
                           let rimLink = data["link"] as? String,
                           let rimPrice = data["price"] as? Double,
                           let rimImage = data["imageName"] as? String {
                            let newRim = Rim(id: rimId, name: rimName, link: rimLink, price: rimPrice, imageName: rimImage)
